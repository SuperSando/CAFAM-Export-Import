...
def log_to_github(filename, process_type):
    try:
        ...
        # 4-column format: Timestamp, Filename, Process, Status
...
with tab2:
    ...
    if uploaded_modlist:
        if st.button("🚀 Process Modlist Items"):
            ...
            light_blue_fill, light_green_fill = PatternFill(start_color='ADD8E6', end_color='ADD8E6', fill_type='solid'), PatternFill(start_color='90EE90', end_color='90EE90', fill_type='solid')
            darker_yellow_fill, center_align = PatternFill(start_color='FFFF99', end_color='FFFF99', fill_type='solid'), Alignment(horizontal='center', vertical='center')

            green_identifiers = ["AD", "SB", "MSB", "SIL", "CSB"]
            yellow_identifiers = ["8.33 KHZ CONVERSION", "AIRCON/SVC.RECHARGE.ANN", "NO LONGER A REQUIREMENT", "CAA SD-2024/001 V.2", "PROPELLER BALANCING", "DELETED", "WHEEL AND BRAKE CONFIRMATION: Please verify the wheel/ brake system"]
            ...
            header_replacements = {"DIRCTVE": "SB/SL", "DESCR": "Description", "GROUP": "Valid", "REASON": "Method of compl.", "DATESAT": "C/W Date", "AC_HRS_SAT": "C/W FH", "AC_LDG_SAT": "C/W FC", "DATE_DUEBY": "Remarks", "ALTREFNO": "AD Foreign", "JOBNO": "C/W WO", "ACTION": "C/W"}
            ...
            for cell in ws[1]:
                if cell.value:
                    ...
                    if hdr in header_replacements:
                        cell.value = header_replacements[hdr]; cell.fill = light_blue_fill; hdr = cell.value
            ...
            for row_idx in range(2, ws.max_row + 1):
                c_sb, c_ad, c_mt = ws.cell(row=row_idx, column=col_sb_sl), ws.cell(row=row_idx, column=col_ad), ws.cell(row=row_idx, column=col_mt)
                c_fh, c_ds, c_dt = ws.cell(row=row_idx, column=col_cw_fh), ws.cell(row=row_idx, column=col_descr), ws.cell(row=row_idx, column=col_cw_date)
                c_cw, c_wo = ws.cell(row=row_idx, column=col_cw_action), ws.cell(row=row_idx, column=col_cw_wo)
                
                v_sb_orig = str(c_sb.value or "").strip()
                if v_sb_orig.upper().startswith("AD"): c_ad.value = c_sb.value; c_sb.value = None
                if c_ad.value: c_mt.value = "AD"
                elif c_sb.value: c_mt.value = "SB/SL"
                if c_fh.value is not None: c_fh.value = format_to_hhmm(c_fh.value)
                if c_dt.value:
                    c_cw.value = "TRUE"; c_dt.number_format = 'yyyy-mm-dd'
                if c_wo.value: c_wo.value = str(c_wo.value).strip().rstrip('/')

                v_sb, v_ad, v_ds = str(c_sb.value or "").upper(), str(c_ad.value or "").upper(), str(c_ds.value or "").upper()
